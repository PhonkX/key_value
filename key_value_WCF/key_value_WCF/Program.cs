...
namespace key_value_WCF
{
    class Program
    {
        static void Main(string[] args)
        {
            try
            {
                using (var fileStream = new FileStream("users.xml", FileMode.Create))
                {
            ...
            {
                Console.WriteLine("Address error. Try to reboot server or start it as administrator.");
            }
            //catch (Exception e)
            //{
            //    Console.WriteLine("Service error. Please, reboot server.");
            //}
        }
    }
}
